...
export default function ProductEnquiry({ product }: Props) {
  ...
  return (
    <div className="border border-steel-light bg-surface-alt p-6 md:p-8">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3">
        <a
          ...
          className="inline-flex items-center justify-center gap-2 bg-signal hover:bg-signal-deep text-white text-sm font-semibold tracking-wider uppercase px-6 py-3.5 transition-colors"
        >
          {t.catalogueWhatsApp}
        </a>
        <a
          href={`tel:${CONTACT.phoneTel[0]}`}
          className="inline-flex items-center justify-center gap-2 border border-ink/15 hover:border-ink/40 text-ink text-sm font-semibold tracking-wider uppercase px-6 py-3.5 transition-colors"
        >
          {t.catalogueCall}
        </a>
        <a
